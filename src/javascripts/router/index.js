...
class Router {
    ...
    refresh() { // 根据当前的路径 来 切换路由
        ...
        //href包含shop_car，不包含#/job
        if (href.indexOf('shop_car.html') != -1 && href.indexOf('#/job') == -1) {
            location.href = href.replace('shop_car.html', '')
        }
        let hash = location.hash;
        /* if (!this.routes[hash]&&href.indexOf('shop_car.html')==-1) { // 路由表里没有配置这个路由
            // 回到默认路由
            location.hash = this.initial;
            return false;
        } */ 
        if (hash === '#/cat') {
            location.href = 'http://localhost:8080/shop_car.html'
        } else if (href == 'http://localhost:8080/shop_car.html#/job') {
        } else {
